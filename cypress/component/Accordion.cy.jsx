...
describe('Accordion.cy.jsx', () => {
  it('Items accordion', () => {
    ...
    cy.getDataTest('accordion-wrapper').as('accordionWrapper')

    // cy.get('@accordionWrapper').children().should('have.length', 8)
    cy.get('@accordionWrapper').within(() => {
      // cy.get('[data-test^="accordion-item"]').should('have.length', 8)
      cy.get('>div').should('have.length', 8)
    })

    // cy.get('@accordionWrapper').within(() => {
    //   cy.get('[data-test="accordion-item-summary-1"]').should('be.visible')
    // })
