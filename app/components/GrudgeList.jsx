...
export default function GrudgeList() {
  ...
  return (
    <div style={{ margin: '20px' }}>
      ...
      <ul data-test="grudge-list" style={{ color: 'white', listStyleType: 'none' }}>
        {
          grudges.length > 0 && (
            grudges.map((g) => {
              return (
                <li data-test="grudge-list-item" key={g.id}>
                  <span>
                    {g.text}
                  </span>
                  <Button data-test="remove-grudge-item-button" onClick={() => deleteGrudge(g)}>
                    X
                  </Button>
                </li>
              )
      ...
      {
        grudges.length > 0 && <Button data-test="remove-all-grudge-item-button" onClick={clearGrudges}>Clear</Button>
      }
    </div>
  )
}